const num: number[] = [4, 5, 6];

num.push(7, 8, 9); // Adiciona valores no final do array
num.unshift(1, 2, 3); // Adiciona valores no início do array
num.splice(2, 0, 'novo valor' as any); // Adiciona valores em uma posição específica do array
...
console.log(movies.includes({ id: 1, title: 'Titanic' })); // .includes não funciona para objetos literais

console.log(movies.find((movie) => movie.title === 'The Matrix')); // Retorna o objeto que atende a condição

// Arrow function
console.log(movies.find(movie => movie.title === 'The Matrix')); // => arrow function

// Removendo itens da Array
const num3: number[] = [1, 2, 3, 4, 5, 6, 7, 8, 9];
...
let all = numbers.concat(letters); // Concatena duas arrays
let half = all.splice(3, 5); // Remove os elementos da posição 3 até a posição 5

console.log(all);
console.log(half);

// Utilizando o Join Array
let clients: string[] = ['Ana', 'Carlos', 'Maria', 'José'];
console.log(clients);
...
let clients2: string[] = ['Ana', 'Carlos', 'Maria', 'José'];
clients2.reverse(); // Inverte a ordem dos elementos da array
